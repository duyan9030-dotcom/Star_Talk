  ...
  const handleVoiceInput = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        const fullBlob = new Blob(chunks.length > 0 ? chunks : [], { type: "audio/webm" });
        ...
        try {
          ...
          const sttResponse: any = await api.speechToText({ audioBase64: base64, mimeType: fullBlob.type });
          const recognized = (sttResponse?.data || sttResponse || "").toString();
          setRecognizedAudioText(recognized);
          setInput(recognized);

          // 自动发送识别结果；如需手动发送可移除下一行
          // await handleSend();
