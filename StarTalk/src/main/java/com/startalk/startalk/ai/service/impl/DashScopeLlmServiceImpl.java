...
public class DashScopeLlmServiceImpl implements LlmService {
    ...
    public String chat(List<ChatMessage> messages) {
        try {
            ...
            if (apiKey == null || apiKey.isEmpty() || apiKey.contains("your-dashscope")) {
                return getMockResponse(messages);
            }
            return callDashScope(buildMessages(messages), null);
        } catch (Exception e) {
            throw new BusinessException("通义千问LLM调用失败: " + e.getMessage());
        }
    }
    ...
    private String callDashScope(List<Map<String, String>> messages, String systemPrompt) {
        try {
            String apiKey = aiConfig.getDashscope().getApiKey();
            String model = aiConfig.getDashscope().getModel();
    ...
    private String getMockResponse(List<ChatMessage> messages) {
        if (messages == null || messages.isEmpty()) {
            return "Hello! I'm 通义千问. How can I help you practice English today?";
        }
